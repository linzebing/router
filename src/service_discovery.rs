        ...
        let pod_type = if let Some(config) = config {
            if config.pd_mode {
                ...
                let prefill_pool = config
                    .prefill_selectors
                    .iter()
                    .find(|(_, sel)| Self::matches_selector(pod, sel))
...
mod tests {
    ...
    fn test_pod_info_equality_with_pod_type() {
        let pod1 = PodInfo {
            ...
            status: "Running".into(),
            is_ready: true,
            pod_type: Some(PodType::Prefill { pool: "default".to_string() }),
    ...
    fn create_labeled_pod(name: &str, ip: &str, labels: Vec<(&str, &str)>, bootstrap_port: Option<u16>) -> Pod {
    ...
    fn test_from_pod_multi_pool_text_prefill() {
        ...
        assert_eq!(
            pod_info.pod_type,
            Some(PodType::Prefill { pool: "text".to_string() })
        );
    ...
    fn test_from_pod_multi_pool_perception_prefill() {
        ...
        assert_eq!(
            pod_info.pod_type,
            Some(PodType::Prefill { pool: "perception".to_string() })
        );
    ...
    fn test_pod_type_equality_different_pools() {
        let text_pool = PodType::Prefill { pool: "text".to_string() };
        let perception_pool = PodType::Prefill { pool: "perception".to_string() };
        let default_pool = PodType::Prefill { pool: "default".to_string() };

        assert_ne!(text_pool, perception_pool);
        assert_ne!(text_pool, default_pool);
        assert_ne!(perception_pool, default_pool);

        let text_pool2 = PodType::Prefill { pool: "text".to_string() };
        assert_eq!(text_pool, text_pool2);
    }

    #[test]
    fn test_overlapping_selectors_deterministic_pool() {
        ...
        // BTreeMap guarantees alphabetical iteration: "perception" is checked before "text",
        // so the pod should always be assigned to "perception".
        ...
        for _ in 0..10 {
            let pod_info = PodInfo::from_pod(&pod, Some(&config)).unwrap();
            // BTreeMap iterates alphabetically: "perception" < "text"
            // So "perception" is always checked first and matches
            assert_eq!(
                pod_info.pod_type,
                Some(PodType::Prefill { pool: "perception".to_string() }),
                "Pool assignment must be deterministic with overlapping selectors"
            );
        }
    }
