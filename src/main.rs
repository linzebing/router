...
fn parse_prefill_selectors(selector_list: &[String]) -> BTreeMap<String, HashMap<String, String>> {
    ...
    for item in selector_list {
        ...
        let pool_name = if pool_name.is_empty() {
            eprintln!("WARNING: Empty pool name in --prefill-selector '{}', using 'default'", item);
            "default"
        } else {
            pool_name
        };
    ...
    // Warn if configured pool names don't match known modality detection outputs
    let known_pools = ["default", "text", "perception"];
    for pool_name in pools.keys() {
        if !known_pools.contains(&pool_name.as_str()) {
            eprintln!(
                "WARNING: Prefill pool '{}' does not match any known modality detection output ({:?}). \
                 Requests will never be routed to this pool unless custom detection is implemented.",
                pool_name, known_pools
            );
        }
    }
...
mod tests {
    ...
    fn test_parse_prefill_selectors_empty_pool_name() {
        ...
        assert_eq!(result.get("default").unwrap().get("app").unwrap(), "my-prefill");
    }
    ...
    fn test_parse_prefill_selectors_whitespace() {
        ...
        let result = parse_prefill_selectors(&selectors);
        assert_eq!(result.len(), 1);
        let text_pool = result.get("text").expect("pool name should be trimmed to 'text'");
        assert_eq!(text_pool.get("app").unwrap(), "prefill");
